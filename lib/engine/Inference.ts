...
export async function generateResponse(swipeId: number) {
    ...
    if (appMode === 'local') {
        ...
        await BackgroundService.start(chatInferenceStreamWithTools, completionTaskOptions)
    }
}
...
async function chatInferenceStreamWithTools() {
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        Logger.info(`[ToolCalling] Generation round ${round + 1}/${MAX_TOOL_ROUNDS}`)

        const result = await runSingleInference(hasTools ? tools : undefined)
        ...
        if (!hasToolCalls(result)) {
            Logger.info('[ToolCalling] No tool calls in response, done')
            break
        }

        const toolCalls = parseToolCalls(result)
        if (toolCalls.length === 0) {
            Logger.info('[ToolCalling] Tool calls parsed but empty, done')
            break
        }

        Logger.info(`[ToolCalling] Found ${toolCalls.length} tool calls, executing...`)

        const toolResults = await executeToolCalls(toolCalls)
        const toolResultText = formatToolResults(toolResults)

        const charName = Characters.useCharacterStore.getState().card?.name ?? 'Assistant'
        const toolResultName = 'System'

        await Chats.useChatState.getState().addEntry(toolResultName, false, toolResultText)

        Logger.info(`[ToolCalling] Tool results added, continuing generation...`)
    }
...
async function runSingleInference(tools?: ReturnType<typeof toolRegistry.getDefinitions>): Promise<string | void> {
    ...
    if (tools && tools.length > 0) {
        const toolsText = formatToolsForPrompt(tools)
        const originalOnEnd = fields.onEnd
        fields.onEnd = async () => {
            await originalOnEnd()
        }

        if (fields.messages && fields.messages.length > 0) {
            const lastMsg = fields.messages[fields.messages.length - 1]
            if (!lastMsg.is_user) {
                const toolSystemMsg = {
                    id: -2,
                    chat_id: -1,
                ...
                fields.messages.push(toolSystemMsg)
            }
        }
    }
